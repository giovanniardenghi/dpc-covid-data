...
sesso_eta['Rimossi'] = sesso_eta['Casi'] + sesso_eta['Deceduti']
Infected = stato_clinico.groupby(level=[1,2]).sum()
#stato_clinico.loc['Infetti'] = stato_clinico.loc['Isolated'] + stato_clinico.loc['Hospitalized'] + stato_clinico.loc['Threatened']
sesso_eta_perc = sesso_eta / sesso_eta.groupby('Data').sum()
tmp_infected = pd.concat({'Infected': Infected['Casi']}, names=['Stato']).to_frame(name='Casi')
stato_clinico = stato_clinico.append(tmp_infected)
stato_clinico_perc = stato_clinico / stato_clinico.groupby(['Stato','Data'],level=[0,1]).sum()

tmp = pd.concat({'Removed': sesso_eta_perc['Rimossi']}, names=['Stato']).to_frame(name='Casi')
stato_clinico.reindex(columns=['Infected'])
...
stato_clinico_perc = stato_clinico_perc.reindex(columns=['Infected', 'Removed'])
...
stato_clinico_perc.reset_index().to_csv('SUIHTER/stato_clinico_SIR.csv',index=None)
